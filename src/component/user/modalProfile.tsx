...
function getMembershipColor(level: MembershipLevel) {
  switch (level) {
    case "Platinum": return "from-cyan-200 via-sky-300 to-indigo-300";
    case "Gold": return "from-yellow-300 via-amber-400 to-orange-400";
    case "Silver": return "from-slate-200 via-slate-300 to-slate-400";
    default: return "from-lime-300 via-emerald-300 to-green-400";
  }
}
...
function getNextLevel(level: MembershipLevel): { next: string; pointsNeeded: number } {
  switch (level) {
    case "Member": return { next: "Silver", pointsNeeded: 2000 };
    case "Silver": return { next: "Gold", pointsNeeded: 5000 };
    case "Gold": return { next: "Platinum", pointsNeeded: 10000 };
    case "Platinum": return { next: "Max", pointsNeeded: 10000 };
  }
}
...
function getUserFromCookies(): UserProfile | null {
  ...
  if (!accessToken || !role || role !== "user") return null;
...
// ── MUI Box style (chỉ định vị, không đổi visual) ──────────────────────────
const boxStyle = {
  position: "absolute" as const,
  top: "50%",
  left: "50%",
  transform: "translate(-50%, -50%)",
  width: { xs: "calc(100% - 32px)", sm: 448 }, // max-w-md tương đương
  outline: "none",                              // bỏ focus ring mặc định của MUI
};
...
export default function ModalProfile({ open, onClose }: ModalProfileProps) {
  ...
  return (
    <>
      {/* ── MUI Modal làm khung (backdrop + portal + a11y) ── */}
      <Modal
        open={open}
        onClose={onClose}
      ...
      >
        <Box sx={boxStyle}>
          {/* ── Nội dung giữ nguyên 100% từ bản gốc ── */}
          <div className="relative z-10 w-full animate-[modalSlideUp_0.35s_ease-out] rounded-2xl border border-white/10 bg-gray-900/95 shadow-[0_30px_100px_rgba(0,0,0,0.6)] backdrop-blur-xl">
            {/* Close Button */}
            <button
              onClick={onClose}
              ...
              <div className="p-8 text-center">
                <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-gray-800">
                  <svg viewBox="0 0 24 24" fill="none" className="h-10 w-10 text-gray-500" aria-hidden="true">
                    <path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm-7 7a7 7 0 0 1 14 0" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
                  </svg>
                </div>
                <h3 id="modal-profile-title" className="mb-2 text-lg font-bold text-white">Chưa đăng nhập</h3>
                <p id="modal-profile-description" className="mb-6 text-sm text-gray-400">
              ...
              <>
                ...
                <div className="relative overflow-hidden rounded-t-2xl p-6 pb-8">
                  ...
                  <div className="relative flex items-start gap-4">
                    <div className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ${getMembershipColor(user.membershipLevel)} text-2xl font-bold text-black shadow-lg`}>
                      {user.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 id="modal-profile-title" className="truncate text-lg font-bold text-white">{user.name}</h3>
                      <p className="mt-0.5 truncate text-sm text-gray-400">{user.email}</p>
                      <div className="mt-2 flex items-center gap-2">
                        <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${getMembershipBadgeColor(user.membershipLevel)}`}>
                        ...
                        {user.provider === "google" && (
                          <span className="inline-flex items-center gap-1 rounded-full border border-blue-500/20 bg-blue-500/10 px-2.5 py-0.5 text-xs font-semibold text-blue-400">
                            <svg viewBox="0 0 24 24" className="h-3 w-3" aria-hidden="true">
                              ...
                              <path d="M5.84 14.09a7.12 7.12 0 0 1 0-4.17V7.07H2.18A11.96 11.96 0 0 0 0 12c0 1.94.46 3.77 1.28 5.4l3.56-2.77.01-.54Z" fill="#FBBC05" />
                ...
                <div className="px-6 pb-4">
                  <div className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <span className="text-lg font-bold text-yellow-500">{user.points.toLocaleString("vi-VN")}</span>
                    </div>
                    ...
                    {nextLevelInfo && user.membershipLevel !== "Platinum" && (
                      <p className="text-xs text-gray-500">
                        Cần thêm{" "}
                        <span className="font-medium text-gray-300">{(nextLevelInfo.pointsNeeded - user.points).toLocaleString("vi-VN")}</span>{" "}
                        điểm để lên <span className="font-medium text-yellow-500">{nextLevelInfo.next}</span>
                      </p>
                    )}
                ...
                <div className="px-6 pb-4">
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3 rounded-lg bg-white/[0.02] px-4 py-3">
                      <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5 shrink-0 text-gray-500" aria-hidden="true">
                        <path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm-7 7a7 7 0 0 1 14 0" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
                      </svg>
                      <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-3 rounded-lg bg-white/[0.02] px-4 py-3">
                      <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5 shrink-0 text-gray-500" aria-hidden="true">
                        <path d="M12 22s-8-4.5-8-11V5l8-3 8 3v6c0 6.5-8 11-8 11Z" stroke="currentColor" strokeWidth="1.6" strokeLinejoin="round" />
                      </svg>
                      <div className="min-w-0 flex-1">
                        <p className="text-xs text-gray-500">Phương thức đăng nhập</p>
                        <p className="text-sm text-white">{user.provider === "google" ? "Google" : "Email / Mật khẩu"}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t border-white/5 px-6 py-4">
                  ...
                  >
                    <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5" aria-hidden="true">
                      <path d="M10 17 15 12 10 7M15 12H4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                      <path d="M13 4h4a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-4" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
                    </svg>
                    Đăng xuất
                  </button>
                </div>
